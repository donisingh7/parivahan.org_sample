  ...
  const handleGetPdf = async () => {
    ...
    try {
      const res = await fetch("/api/payment", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ transactionId, state: stateCode, visitingState: stateCode, vehicleNo, chassisNo, ownerName, mobileNo, fromState, vehicleType, vehicleCategory, vehicleClass, grossVehicleWt, unladenWt, serviceType, taxMode, borderDistrict, checkpostName, taxFrom, taxFromTime, taxTo, taxToTime, amount: parseFloat(effectiveTotal)||0, userCharge: userCharge||"0", infraCess: infraCess||"0", receiptNo, orderRef: (() => { const numD=Math.floor(Math.random()*3)+1; const numL=10-numD; const c=[]; for(let i=0;i<numL;i++)c.push(String.fromCharCode(65+Math.floor(Math.random()*26))); for(let i=0;i<numD;i++)c.push(String(Math.floor(Math.random()*10))); for(let i=c.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[c[i],c[j]]=[c[j],c[i]];} return c.join(""); })(), noOfPeriods:1, seatingCap, sleeperCap, paymentInitDate: paymentInitDateInput || nowIST(), paymentConfDate: paymentConfDateInput, printedOn: printedOnInput }) });
      const json = await res.json().catch(()=>({}));
