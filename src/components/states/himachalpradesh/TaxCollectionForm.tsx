...
// ── Circular clock picker (Android-style) ─────────────────────────────────
...
function ClockPicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const selPos = mode === "hour"
    ? clockPos(HOUR_SLOTS.indexOf(hour), 12)
    : clockPos(minute / 5, 12);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && (
        ...
        >
          <div
            style={{ background: "#fff", borderRadius: "16px", overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.3)", width: "292px" }}
          ...
          >
            ...
            <div style={{ background: CLOCK_ACCENT, color: "#fff", padding: "18px 22px 14px" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: "8px" }}>
                <div style={{ fontSize: "46px", fontWeight: 300, lineHeight: 1, letterSpacing: "1px" }}>
                  <span
                    style={{ cursor: "pointer", opacity: mode === "hour"   ? 1 : 0.6, borderBottom: mode === "hour"   ? "2px solid #fff" : "none" }}
                    onClick={() => setMode("hour")}
                  >{String(hour).padStart(2, "0")}</span>
                  <span style={{ opacity: 0.75 }}>:</span>
                  <span
                    style={{ cursor: "pointer", opacity: mode === "minute" ? 1 : 0.6, borderBottom: mode === "minute" ? "2px solid #fff" : "none" }}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "2px", marginBottom: "7px", fontSize: "13px", fontWeight: 600 }}>
                  {(["AM", "PM"] as const).map((ap) => (
                    <span
                      key={ap}
                      style={{ cursor: "pointer", opacity: ampm === ap ? 1 : 0.55, padding: "1px 5px", borderRadius: "3px", background: ampm === ap ? "rgba(255,255,255,0.25)" : "transparent" }}
                      onClick={() => setAmpm(ap)}
                    >{ap}</span>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", justifyContent: "center", padding: "14px 8px 6px", background: "#f7f7f7" }}>
              <svg width={CLOCK_SIZE} height={CLOCK_SIZE} onClick={handleClockClick} style={{ cursor: "crosshair", display: "block" }}>
                ...
                {mode === "hour"
                  ? HOUR_SLOTS.map((h, i) => {
                      const p = clockPos(i, 12);
                      return (
                        <text key={h} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="central"
                          fontSize="13" fontWeight={h === hour ? "bold" : "normal"}
                          fill={h === hour ? "#fff" : "#333"}
                          style={{ userSelect: "none", pointerEvents: "none" }}
                        >{h}</text>
                      );
                    })
                  : MINUTE_SLOTS.map((m, i) => {
                      const p = clockPos(i, 12);
                      return (
                        <text key={m} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="central"
                          fontSize="13" fontWeight={m === minute ? "bold" : "normal"}
                          fill={m === minute ? "#fff" : "#333"}
                          style={{ userSelect: "none", pointerEvents: "none" }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 18px 14px", background: "#fff" }}>
              <button type="button"
                style={{ background: "none", border: "none", color: CLOCK_ACCENT, fontWeight: 700, cursor: "pointer", fontSize: "14px" }}
                onClick={() => { setHour(12); setMinute(0); setAmpm("AM"); setMode("hour"); }}
              >Clear</button>
