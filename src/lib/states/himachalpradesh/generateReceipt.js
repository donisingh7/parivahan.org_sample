/**
 * Himachal Pradesh PDF receipt generator. Each state has its own copy of this file
 * under src/lib/states/<state>/generateReceipt.js so the per-state design is
 * editable in isolation. The state-name strings ("GOVERNMENT OF HIMACHAL PRADESH",
...
const STATE_GOV_LABEL    = 'GOVERNMENT OF HIMACHAL PRADESH';
...
/**
 * Generates a Himachal Pradesh checkpost tax receipt PDF and returns it as a Buffer.
...
async function generateReceipt(data) {
  ...
  // Row 1: Registration No. (bold) + Receipt No.
  doc.fontSize(fieldFontSize).font('Helvetica').fillColor('#000000');
  ...
  // HP inspect HTML rows 4683-4756 — eleven additional two-column rows.
  ...
  // ===== 6. NOTES (HP inspect HTML lines 4853-4892) ====================
  doc.fontSize(10.5).font('Helvetica-Bold').fillColor('#000000');
