/**
 * Single source of truth for the Gujarat checkpost-tax receipt.
 *
...
export function buildGujaratReceiptData(txn: TxnLike): ReceiptData {
  ...
  // Read GJ-specific fields from the document record.
  const gj = txn as Record<string, unknown>;
  const mvTax     = Number(gj.gjMvTax)    || 0;
  const permitFee = Number(gj.gjPermitFee) || 0;
  ...
  return {
    ...
    permitCategory:  "",
    makerStatus:     (gj.gjMakerStatus as string) || "",
    ...
    unladenWeight:  (gj.gjUnladenWeight as string) || "-",
    ...
    cap1Value: (String(txn.vehicleType || '')).toUpperCase() === 'GOODS VEHICLE' ? (Number(gj.gjLadenWeight as string) || 0) : (Number(txn.seatingCap) || 0),
    cap2Value: (String(txn.vehicleType || '')).toUpperCase() === 'GOODS VEHICLE' ? (Number(gj.gjUnladenWeight as string) || 0) : (Number(txn.sleeperCap) || 0),
  };
}
