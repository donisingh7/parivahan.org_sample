/**
 * Gujarat PDF receipt generator — adapted from the reference design.
...
async function generateReceipt(data) {
  ...
  const logoPath = path.join(process.cwd(), 'public', 'Images', 'Gujarat.png');
  ...
  const pageHeight   = 841.89;   // eslint-disable-line @typescript-eslint/no-unused-vars
  const margin       = 30;
  ...
  // -- Watermark image --------------------------------------------------------
  // Gujarat emblem is scaled 1.7x (both width & height) relative to the
  // Maharashtra watermark, kept as an upper-background element and nudged
  // slightly to the right.
  const wmBuffer = await loadGreyscaleImage(logoPath);
  if (wmBuffer) {
    const wmWidth  = 205 * 1.7;   // 1.7x MH width
    const wmHeight = 185 * 1.7;   // 1.7x MH height
    const wmX = (pageWidth - wmWidth) / 2 + 25;  // centered, nudged slightly right
    const wmY = 30;                              // upper background, fully on-page
    doc.save();
    doc.opacity(0.7);
  ...
  doc.text('GOVERNMENT OF GUJARAT', titleX, y + 2,
  ...
  // Row 10 - two columns: Service Type | Maker Status
  // Maker Status sits below Payment Mode (right column of Row 9) and above
  // Permit Category (right column of Row 11) — GJ-specific field.
  const row10Y = y;
  drawField('Service Type', data.serviceType || 'NOT APPLICABLE', col1X, row10Y);
  drawField('Maker Status', data.makerStatus || '-',              col2X, row10Y);
  y += fieldLineHeight;
