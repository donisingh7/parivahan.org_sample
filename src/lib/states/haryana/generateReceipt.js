/**
 * Haryana PDF receipt generator. Each state has its own copy of this file
 * under src/lib/states/<state>/generateReceipt.js so the per-state design is
 * editable in isolation.
 *
 * The layout mirrors GOVERNMENT OF HARYANA's Checkpost Tax e-Receipt:
 *   • Watermark = HR transport emblem (tiled "<RegNo> <Date>" overlay).
 *   • Header = emblem on the left, gov/dept/receipt headings centre, QR
 *     on the right with "Printed on : …" above it.
 *   • Two-column field grid (no Surcharge row, single MV Tax line).
 *   • Notes & Terms-and-Conditions tied to the Haryana inspect HTML.
 *
...
const STATE_GOV_LABEL      = 'GOVERNMENT OF HARYANA';
const STATE_DEPT_LABEL     = 'Department of Transport';
const STATE_RECEIPT_TITLE  = 'Checkpost Tax e-Receipt';
const STATE_WATERMARK_FILE = 'Haryana-Transport-Department.png';
...
// the user is still sourcing the Haryana emblem image).
...
/**
 * Generates a Haryana checkpost tax receipt PDF and returns it as a Buffer.
 * @param {Object} data - Booking/payment data produced by buildHaryanaReceiptData.
