/**
 * Odisha PDF receipt generator (placeholder cloned from the Andhra Pradesh
 * design; will be replaced by the exact Odisha receipt code later).
...
async function generateReceipt(data) {
  ...
  const logoPath = path.join(process.cwd(), 'public', 'Images', 'Rajasthan-Transport-Department.png');
  ...
  if (fs.existsSync(logoPath)) {
    ...
    doc.text('GOVT. OF\nODISHA', emblemX + 10, emblemY + 28, { width: 50, align: 'center' });
    doc.restore();
  }
  ...
  doc.text('GOVERNMENT OF ODISHA', titleX, y + 55, { width: titleWidth, align: 'center', underline: true });
  doc.fontSize(11).font('Helvetica-Bold');
  doc.text('Odisha Motor Vehicles Department', titleX, y + 85, { width: titleWidth, align: 'center' });
