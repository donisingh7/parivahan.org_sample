...
async function loadGreyscaleImage(imagePath) {
  const buffer = await sharp(imagePath)
    ...
    .png()         
    .blur(1.7)         // ← converts webp to PNG (PDFKit needs this)
    .toBuffer();
  return buffer;
}
...
/**
 * Main receipt generator function
...
// 1. IMAGE WATERMARK
// =====================================================
const imagePath = path.join(__dirname, '..', 'public', 'Images', 'Rajasthan-Transport-Department.png');
...
doc.save();
doc.opacity(0.75);   // ← CHANGE THIS — lower = more faint (0.05 to 0.15 recommended)
doc.image(wmBuffer, wmX-10, wmY - 230, {   // ← minus 80 shifts it UP by 80pts
  width: wmWidth,
  height: wmHeight,
});
doc.restore();
...
y += fieldLineHeight * 1.5;
  ...
  for (const item of taxItems) {
      // Remove watermark only behind these two rows
    if (
      item.particular === 'MV Tax(17-Apr-2026)' ||
