...
async function drawImageWatermark(doc, imagePath, pageWidth, pageHeight) {
  ...
  const wmWidth  = 230;  // ← size
  const wmHeight = 245;  // ← size
  const wmX      = 305;  // ← left se position
  const wmY      = 188;  // ← upar se position
...
async function generateReceipt(data, outputPath) {
  ...
  const qrUrl    = data.qrUrl || `https://himachalparivahan.gov.in/verify?receipt=${data.receiptNo}`;
  const qrBuffer = await generateQRCode(qrUrl);

  const logoPath = data.emblemImagePath || './images/HP_logo.png';
  ...
  if (fs.existsSync(logoPath)) {
    ...
    const logoW = 100;   // ← width
    const logoH = 100;   // ← height
  ...
  drawField('PUCC\nValidity',                data.puccValidity      || '-', col2X, y);
  y += fh2;
...
for (const item of page1Items) {
  ...
  y += firstRowHeight;  // ← firstRowHeight use ho raha hai
  doc.moveTo(margin - 2, y).lineTo(pageWidth - margin + 2, y).lineWidth(0.5).stroke(borderColor);
}
...
// ── Continued tax table (remaining items) ─────────────────
...
for (const item of page2Items) {
  const textY2 = y2 + (rowHeight / 2) - 6;  // ← vertically center
  ...
  // ← Har row ke baad separator line — yeh fix karta hai missing border
  doc.moveTo(margin - 2, y2).lineTo(pageWidth - margin + 2, y2).lineWidth(0.5).stroke(borderColor);
}
drawTableBorders(doc, tableStartY2, y2);
  y2 += 15;
