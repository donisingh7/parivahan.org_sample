...
function drawText(doc, value, x, y, options = {}) {
  ...
  const selectedFont = options.font || 'Roboto';
  doc.save();
  doc.translate(x, y);
  doc.scale(scaleX, 1);
  doc
...
function drawReferencePath(doc, name) {
  ...
  if (!entry) throw new Error(`Missing reference vector path: ${name}`);
...
function drawImages(doc, data, qrBuffer) {
  ...
  doc.image(data.sealImagePath, 165.934021, 209.68399, {
    width: 255.208312,
    height: 262.5
  });
  doc.restore();

  doc.image(data.sealImagePath, 27.392361, 12.80896, {
    width: 109.374989,
    height: 109.375
  });
  doc.image(qrBuffer, 460.805817, 12.617081, {
    width: 110.910064,
    height: 109.95066
  });
}
...
function drawHeader(doc, data) {
  ...
  drawText(doc, 'Motor Vehicles Department', 205.807281, 40.419834, {
    size: 15.3125
  });
  drawText(doc, 'Checkpost Tax e-Receipt', 227.244797, 69.426811, {
    size: 13.125
  });
  drawText(
    doc,
...
function drawDetails(doc, data) {
  ...
  drawField(
    ...
    110.602417,
    268.466187,
    { valueYOffset: 6.319458, lineHeight: 12.638889 }
  );
}
...
function drawTable(doc, data) {
  ...
  const rows = [313.188416, 326.313416, 339.438416];
  const items = data.taxItems || [];
  rows.forEach((y, index) => {
    const item = items[index] || {};
    drawText(doc, item.particular || '', 27.392361, y, { size: 9.479 });
    drawText(doc, item.fees || '', 397.262146, y, { size: 9.479 });
    drawText(doc, item.fine || '', 458.905151, y, { size: 9.479 });
    drawText(doc, item.total || '', 520.539917, y, { size: 9.479 });
  });
}

function drawGrandTotal(doc, data) {
  drawReferencePath(doc, 'grandLabel');
  doc.image(data.rupeeGlyphPath, 78.75, 366.515533, {
    width: 7.777779,
    height: 7.792969
  });
  if (
...
function drawPermit(doc, data) {
  ...
  drawText(doc, 'See Rule 46(1)(v)', 232.397568, 416.183716, {
    size: 15.3125
  });
  drawText(
    doc,
    `TEMP. PERMIT No.: ${permit.number || data.receiptNo}`,
    172.666672,
    445.190704,
    { size: 13.125 }
  );
  ...
  const y = {
    ...
    goods2: 567.128235,
    expiry: 578.794922,
    provision1: 590.461548,
    provision2: 602.128235,
    provision3: 613.794922,
    regions: 625.461548,
    routes: 637.128235
  };

  drawField(doc, '1. Name of Holder', permit.holderName, labelX, valueX, y.holder, {
    valueScaleX: 1,
    size: 8.75
  });
  ...
  drawText(
    doc,
    permit.natureOfGoods ? `: ${permit.natureOfGoods}` : ':',
    valueX,
    561.294922,
    { size: 8.75 }
  );
  drawField(doc, '6. Date of expiry', permit.expiryDate, labelX, valueX, y.expiry, {
    size: 8.75
  });
  drawText(
    doc,
    '7. Under the provisions of Sub-section(4)of Sec 63 of the Act and with the particular general',
    labelX,
    y.provision1,
    { size: 8.75 }
  );
  drawText(
    doc,
    '  consent of the Transport Authority concerned, this',
    labelX,
    y.provision2,
    { size: 8.75 }
  );
  drawText(
    doc,
    '  Permit is valid also in the following regions:',
    labelX,
    y.provision3,
    { size: 8.75 }
  );
  ...
  drawField(doc, '8. Route or Routes', permit.routes, labelX, valueX, y.routes, {
    size: 8.75
  });
...
function drawSecondPage(doc) {
  ...
  drawText(doc, 'https://', 333.071167, 17.982403, {
    size: 10.9375
  });
  drawRule(doc, 332.986115, 29.40966, 346.111115);
  drawRule(doc, 348.541656, 29.40966, 368.472229);

  drawText(doc, 'kms.parivahan.gov.in', 27.392361, 32.565739, {
    size: 10.9375
  });
...
async function enableImageInterpolation(pdfPath) {
  const source = await fs.promises.readFile(pdfPath);
  const pdf = await PDFLibDocument.load(source);
  for (const [, object] of pdf.context.enumerateIndirectObjects()) {
    if (!(object instanceof PDFRawStream)) continue;
    if (object.dict.get(PDFName.of('Subtype')) !== PDFName.of('Image')) continue;
    object.dict.set(PDFName.of('Interpolate'), PDFBool.True);
  }
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        ...
        Creator: 'Odisha Receipt Generator'
      }
    });
    const stream = fs.createWriteStream(outputPath);
    stream.on('finish', async () => {
      try {
        await enableImageInterpolation(outputPath);
        resolve(outputPath);
      } catch (error) {
        reject(error);
      }
