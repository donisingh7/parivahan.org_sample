...
function drawTextWatermark(doc, watermarkText, pageWidth) {
  doc.save();
  doc.opacity(0.5);
  doc.fontSize(10.5).fillColor('#aaaaaa').font('Helvetica');
  const startX   = 30;
  const startY   = 40;
  const endY     = 360;
  const tileStep = 20;
  for (let row = 0; ; row++) {
    ...
    if (rowY > endY) break;
    const lineText = `${watermarkText}  `.repeat(3);
...
async function drawImageWatermark(doc, imagePath, pageWidth, pageHeight) {
  ...
  const wmWidth  = 120;  // ← size
  const wmHeight = 160;  // ← size
  const wmX      = ((pageWidth - 200) / 2)+34;   // ← center
  const wmY      = ((pageHeight - 200) / 2 )-165;  // ← center
  doc.save();
  doc.opacity(0.3);
...
async function generateReceipt(data, outputPath) {
  ...
  const watermarkDateTime = now.format('DD/MM/YY, hh:mm A');
  ...
  const logoPath = data.emblemImagePath || './images/Bihar_logo.png';
  ...
  function drawTableRow(doc, item, y, rHeight) {
    const textY = y + (rHeight / 2) - 6;
  ...
  doc.text(watermarkDateTime, margin, y-20, {
    align: 'left'
  });
  ...
  if (fs.existsSync(logoPath)) {
    const logoX = emblemX + 50;
    const logoY = emblemY + 5;
    const logoW = 50;   // ← width
    const logoH = 65;   // ← height
  ...
  const titleWidth = contentWidth - 190;

  doc.fontSize(9).font('Helvetica-Bold').fillColor('#000000');
  doc.text('GOVERNMENT OF BIHAR', titleX, y + 25, { width: titleWidth, align: 'center', underline: true });

  doc.fontSize(9).font('Helvetica-Bold');
  doc.text('Department of Transport', titleX, y + 39, { width: titleWidth, align: 'center' });

  doc.fontSize(8).font('Helvetica-Bold');
  doc.text('Checkpost Tax e-Receipt', titleX, y + 51, { width: titleWidth, align: 'center' });

  // ── QR Code (top right) ───────────────────────────────────
  const qrW = 90;
  const qrH = 90;
  const qrX = pageWidth - margin - qrW - 100;
  const qrY = y;
  ...
  // Row 5 — Bihar has Vehicle Category
  ...
  drawField('Gross Vehicle Wt(In.\nKg)',     String(data.grossVehicleWt ?? ''), col2X, y);
  y += fh2;

  // Row 7
  drawField('Unladen Wt(In Kg.)',            String(data.unladenWt ?? 0), col1X, y);
  ...
  // Row 11 — Bihar specific: Gross Combination Weight
  drawField('Gross Combination\nWeight(in kg.)', String(data.grossCombinationWeight ?? 0), col1X, y);
  ...
  taxItems.forEach((item, index) => {
    const rHeight = index === 0 ? firstRowHeight : rowHeight;
    y = drawTableRow(doc, item, y, rHeight);
    doc.moveTo(margin - 2, y).lineTo(pageWidth - margin + 2, y).lineWidth(0.5).stroke(borderColor);
  });
