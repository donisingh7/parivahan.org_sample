...
function drawTextWatermark(doc, watermarkText, pageWidth, limitY) {
  ...
  const endY = limitY;   // ← caller decide karega kahan tak aana hai
  const startX = 25;
...
async function generateReceipt(data, outputPath) {
  ...
  // Payment Mode row (~y 541-591) tak hi watermark — fixed limit
  drawTextWatermark(doc, watermarkText, pageWidth, 525);
...
doc.text('Tax/Fee Particular', col.particular + 4, y2 + 9, { width: col.fees - col.particular - 10 });
doc.text('Tax/Fees', col.fees, y2 + 9, { width: 60 });
doc.text('Fine', col.fine, y2 + 9, { width: 40 });
doc.text('Total', col.total, y2 + 9, { width: 50 });
y2 += headerHeight;
